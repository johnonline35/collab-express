 ...
 * property in code.
 */
type StringPath<K extends string | number, V> = V extends Primitive ? `${K}` : `${K}` | `${K}.${Path<V>}`;
/**
 * Given an object or array, recursively return all string paths used to access
 * properties within those objects.
 */
type Path<T> = T extends Array<infer V> ? StringPath<number, V> : {
...
export type ObjectAssign<TArr, TAcc = {}> = TArr extends [
    infer TFirst,
    ...infer TRest
] ? Simplify<ObjectAssign<TRest, Omit<TAcc, keyof TFirst> & TFirst>> : TAcc;
export {};
