...
const assignWorkspaceLead = async (email, workspaceId) => {
  try {
    ...
    // Fetch matching meeting attendees
    const { data: matchingMeetingAttendeesData, error: matchingError } =
      await supabase
    ...
    if (matchingError) {
      console.error(
        "Error fetching matching meeting attendees:",
        matchingError
      );
      return;
    }

    const matchingMeetingAttendees = matchingMeetingAttendeesData || [];

    console.log("matchingMeetingAttendees:", matchingMeetingAttendees);
