    ...
    if (attendeesForThisMeeting.length > 0) {
      ...
      for (let attendee of attendeesForThisMeeting) {
        ...
        if (
          ...
          const workspaceId = existingAttendeesMap.has(attendee.email)
            ? existingAttendeesMap.get(attendee.email).workspace_id
            : existingDomainsMap.get(attendeeDomain);
      ...
      if (workspaceIds.size > 1) {
        // Workspace ID collision detected. Handle the collision as per the new specifications.
        const attendeeDomains = attendeesForThisMeeting.map(
          (attendee) => attendee.email.split("@")[1]
        );
        const existingDomainWorkspaceId = Array.from(workspaceIds).find((id) =>
          attendeeDomains.includes(id)
        );

        if (existingDomainWorkspaceId) {
          // If one of the attendee's domain matches with the existing domain workspace ID
          workspaceId = existingDomainWorkspaceId;
        } else {
          // If none of the attendees' domains match with the existing domain workspace ID
          let leadAssigned = assignWorkspaceLead(
          ...
          workspaceId =
            existingAttendeesMap.get(leadAssigned.email)?.workspace_id ||
            uuidv4();
          if (!existingAttendeesMap.has(leadAssigned.email)) {
            workspacesToCreate.push({
              workspace_id: workspaceId,
              workspace_name: workspaceInfo.workspaceName,
              collab_user_id: userId,
        ...
        workspaceId = Array.from(workspaceIds)[0]; // Single workspace id
      } else {
        // No workspace ID found for any attendee, follow the existing process
        let leadAssigned = assignWorkspaceLead(
          attendeesForThisMeeting,
          meeting
        );
